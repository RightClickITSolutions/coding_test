...
def find_word(puzzle, word):
    ...
    for row in range(18):
        for col in range(18):
            if col <= 18 - word_length:
                # Check for the word horizontally
                if ''.join(puzzle[row][col:col+word_length]) == word:
                    return f"Found the word {word} starting at row {row}, column {col}."
            if row <= 18 - word_length:
                # Check for the word vertically
                if ''.join([puzzle[row+i][col] for i in range(word_length)]) == word:
                    return f"Found the word {word} starting at row {row}, column {col}."
