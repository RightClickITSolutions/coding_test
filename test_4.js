function findWordInMatrix(matrix, word) {
    ...
    for (let i = 0; i < matrix.length; i++) {
        for (let j = 0; j < matrix[i].length; j++) {
            // check if the first letter of the word is found in the matrix
            if (matrix[i][j] === word[0]) {
                // check if the rest of the word can be found in the matrix starting from the current position
...
function checkWord(matrix, i, j, word) {
    ...
    for (let k = 0; k < direction.length; k++) {
        ...
        // check if the current position is within the matrix boundary
        while (x >= 0 && x < matrix.length && y >= 0 && y < matrix[0].length) {
            // check if the current letter in the word matches the letter at the current position in the matrix
...
const tableLength = 18; // variable to store the length of the table
const tableHeight = 18; // variable to store the height of the table
